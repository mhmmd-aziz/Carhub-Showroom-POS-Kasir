...
<head>
  <meta charset="UTF-8">
  <title>Faktur Pembelian - INV-B-<?= str_pad($pembelian['id_pembelian'], 4, '0', STR_PAD_LEFT) ?></title>
...
<body class="bg-gray-100 p-8">
  ...
  <div class="max-w-4xl mx-auto bg-white p-10 shadow-sm border border-gray-200 print:shadow-none print:border-none print:p-0" id="document-content">
    ...
    <div class="flex justify-between items-start border-b border-gray-200 pb-6 mb-6">
      <div>
        <h1 class="text-3xl font-bold text-gray-900 tracking-tight">FAKTUR PEMBELIAN</h1>
        <p class="text-sm text-gray-500 mt-1">No. Invoice: <span class="font-mono font-medium text-gray-900">INV-B-<?= str_pad($pembelian['id_pembelian'], 4, '0', STR_PAD_LEFT) ?></span></p>
        <p class="text-sm text-gray-500">Tanggal: <?= date('d M Y', strtotime($pembelian['tgl_pembelian'])) ?></p>
      </div>
    ...
    <div class="flex justify-between mb-8">
      <div>
        <h3 class="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2">Ditagihkan Kepada (Supplier):</h3>
        <p class="text-base font-medium text-gray-900"><?= $pembelian['nama_supplier'] ?></p>
      </div>
      <div class="text-right">
        <h3 class="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2">Status Pembayaran:</h3>
        <?php if($pembelian['status_pembayaran'] == 'selesai'): ?>
    ...
    <table class="w-full text-left border-collapse mb-8">
      ...
      <tbody>
        <tr class="border-b border-gray-100">
          <td class="py-4 px-4">
            <p class="font-medium text-gray-900"><?= $pembelian['nama_mobil'] ?></p>
            <p class="text-sm text-gray-500">Plat: <?= $pembelian['no_polisi'] ?></p>
          </td>
          <td class="py-4 px-4 text-center">1</td>
          <td class="py-4 px-4 text-right font-mono text-gray-700">Rp <?= number_format($pembelian['harga_beli_beli'], 0, ',', '.') ?></td>
          <td class="py-4 px-4 text-right font-mono font-medium text-gray-900">Rp <?= number_format($pembelian['harga_beli_beli'], 0, ',', '.') ?></td>
        </tr>
      </tbody>
    </table>
    ...
    <div class="flex justify-end mb-12">
      <div class="w-1/2 max-w-sm">
        <div class="flex justify-between py-2 border-b border-gray-200">
          ...
          <span class="font-mono text-gray-900">Rp <?= number_format($pembelian['harga_beli_beli'], 0, ',', '.') ?></span>
        </div>
        <div class="flex justify-between py-3 border-b-2 border-gray-800">
          ...
          <span class="text-lg font-mono font-bold text-gray-900">Rp <?= number_format($pembelian['harga_beli_beli'], 0, ',', '.') ?></span>
        </div>
        <?php if($pembayaran): ?>
        <div class="flex justify-between py-2 text-green-600">
          <span class="text-sm font-medium">Telah Dibayar (<?= date('d/m/Y', strtotime($pembayaran['tgl_bayar'])) ?>)</span>
          <span class="font-mono">- Rp <?= number_format($pembayaran['jumlah_bayar'], 0, ',', '.') ?></span>
        </div>
        <?php endif; ?>
      </div>
    </div>
    ...
    <div class="flex justify-between text-center pt-8">
      <div>
        <p class="text-sm text-gray-500 mb-16">Pihak Showroom,</p>
        <p class="font-medium text-gray-900 underline decoration-gray-300 underline-offset-4">( AutoDesk Showroom )</p>
      </div>
      <div>
        <p class="text-sm text-gray-500 mb-16">Pihak Supplier,</p>
        <p class="font-medium text-gray-900 underline decoration-gray-300 underline-offset-4">( <?= $pembelian['nama_supplier'] ?> )</p>
      </div>
    </div>
  ...
  <script>
    function downloadPDF() {
      ...
      const opt = {
        margin:       10,
        filename:     'Faktur_Pembelian_INV-B-<?= str_pad($pembelian['id_pembelian'], 4, '0', STR_PAD_LEFT) ?>.pdf',
    ...
    // Opsional: Langsung munculkan dialog print saat dibuka
